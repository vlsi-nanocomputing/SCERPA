...
DEBUG_matrix = zeros(stack_mol.num, stack_mol.num + 4);


for DEBUG_ii = 1:stack_mol.num %charge update
%     [P1, P2, P3, P4] = SearchValues( Vout(DEBUG_ii), stack_mol.stack(DEBUG_ii).clock, CK );
    [Q1, Q2, Q3, Q4] = applyTranschar(Vout(DEBUG_ii),stack_mol.stack(ii_mol).clock,CK);
...
for DEBUG_ii = 1:stack_mol.num
    ...
    %evaluate driver
    DEBUG_matrix(DEBUG_ii,1) = sum(pre_driver_effect(:,DEBUG_ii));
    
    %contibutions
    for DEBUG_jj = 1:stack_mol.num
        if DEBUG_jj~=DEBUG_ii
            DEBUG_matrix(DEBUG_ii,1+DEBUG_jj) = ChargeBased_CalPotential(stack_mol.stack(DEBUG_jj), stack_mol.stack(DEBUG_ii));
        end
    end
    
    %evaluated Vout
    DEBUG_matrix(DEBUG_ii,stack_mol.num+2) = sum(DEBUG_matrix(DEBUG_ii,1:stack_mol.num+1));
    
    %algorithm Vout
    DEBUG_matrix(DEBUG_ii,stack_mol.num+3) = Vout(DEBUG_ii);
    
    %error
    DEBUG_matrix(DEBUG_ii,stack_mol.num+4) = Vout(DEBUG_ii) - DEBUG_matrix(DEBUG_ii,stack_mol.num+2);
%     %algorithm Charges
