...
for i=1:12:size(raw_mol,1)
    ...
    for l=1:4
        ...
        stack_mol.stack(k).position = raw_mol(i, 5);
        stack_mol.stack(k).molType = raw_mol(i+1,5);
        stack_mol.stack(k).Vext = raw_mol(i+3,5);
        stack_mol.stack(k).identifier_qll = raw_mol(i+5,5);
        stack_mol.num = k;
        k = k + 1;
end
...
for i=1:8:size(raw_dr,1)   
    ...
    for l=1:4
        ...
        stack_driver.stack(k).position = raw_dr(i, 5);
        stack_driver.stack(k).molType = raw_dr(i+1, 2);
        stack_driver.stack(k).identifier_qll = raw_dr(i+1,5); 
        stack_driver.num = k;
        k = k + 1;
end
