...
%                 plot_evaluationMol(find(activeListMolecule==1)) = 1;
    ...
    %     plots
            ...
            figure(250000000), clf, hold on
                plot(preV_afterVoltageVariation,'-b.','Linewidth',5,'Markersize',15)
                plot(newVout_wodamping,'-r.','Linewidth',5,'Markersize',15)
                plot(V_driver,'--g.','Linewidth',5,'Markersize',15)
                plot([1 stack_mol.num],[0 0],'--k')
                legend ('previous step voltage','step voltage','Driver effect','Location','south')
                grid on, grid minor
                axis([1 stack_mol.num -Inf +Inf])
                xlabel('Molecule, i')
                ylabel('Input Voltage, V_{in,i} [V]')
